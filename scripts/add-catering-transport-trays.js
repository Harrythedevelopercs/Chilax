...
async function run() {
  ...
  let subCat = categories.find(
    (c) =>
      ...
      (c.id === 679 || c.name.toLowerCase() === "catering transport trays" || c.slug === "catering-transport-trays")
  );

  if (!subCat) {
    console.log("Catering Transport Trays subcategory not found under Restaurant! Creating...");
    subCat = await request("/products/categories", "POST", {
      name: "Catering Transport Trays",
      parent: parentCat.id,
      slug: "catering-transport-trays",
      description: "Convenient transport trays for large food orders and catering events.",
    });
  ...
  const CATERING_TRAYS_ID = subCat.id;

  const DEFAULT_IMAGE = "https://media.pakfactory.com/catalog/product/cache/e110e1b4adf17fc607368e660f6aa2d4/f/0/f005-d.jpg";

  const PRODUCTS_TO_ADD = [
    {
      name: "Custom Matchbox Transport Catering Trays",
      sku: "CBE319",
      url: "https://pakfactory.com/custom-matchbox-transport-catering-trays.html",
      description: "Heavy-duty corrugated matchbox style transport catering trays engineered for stackable large food deliveries and event catering.",
      short_description: "Custom matchbox transport catering trays.",
    },
  ];
  ...
  for (const item of PRODUCTS_TO_ADD) {
    ...
    if (existing) {
      ...
      const updatedCatIds = Array.from(new Set([...currentCatIds, RESTAURANT_PARENT_ID, CATERING_TRAYS_ID]));
      ...
      if (needCatUpdate || needSkuUpdate) {
        ...
        if (res && res.id) {
          console.log(`✅ Assigned existing product ID ${existing.id} ("${item.name}") to Catering Transport Trays!`);
        } else {
          console.error(`❌ Failed to update categories for ID ${existing.id}:`, res);
        }
      } else {
        console.log(`ℹ️ Product ID ${existing.id} ("${item.name}") is already assigned to Catering Transport Trays.`);
      }
    } else {
      ...
      const createPayload = {
        ...
        categories: [{ id: RESTAURANT_PARENT_ID }, { id: CATERING_TRAYS_ID }],
        images: [
          {
            src: DEFAULT_IMAGE,
          },
      ...
      if (res && res.id) {
        console.log(`✨ Successfully CREATED product ID ${res.id} (SKU: ${targetSku}, Name: "${item.name}") in Catering Transport Trays!`);
      } else {
  ...
  console.log("\nFinished adding all 1 products to Custom Restaurant Packaging -> Catering Transport Trays!");
}
